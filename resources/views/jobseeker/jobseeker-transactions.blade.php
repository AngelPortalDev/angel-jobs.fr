							...
							<div class="job-bx table-job-bx clearfix" style="overflow-x: auto;">
								...
								<table style="text-wrap: nowrap;">
									...
									<tbody>
										...
										<tr>
											<td class="order-id text-primary">
												@if($records->status === 3)
												<a href="{{route('receipt',['id' => base64_encode($records->id)])}}" target="_blank">{{$records->order_id}}</a>
												@else
												{{$records->order_id}}
												@endif
											</td>
											<td class="job-name">{{$records->plan_name}}</td>
											<td class="amount text-primary"><i class="fa fa-indian-rupee-sign" style="font-size: 12px"></i> {{$records->payment_amount}}</td>
											...
											<span class="badge bg-success" style="font-size: 12px;">Success</span>
											@endif
										 </td>
