...
class employerProfile extends Controller
{
    ...
    public function jobApplications($job_id = "")
    {
        ...
        if (session()->has('emp_username')) {
            ...
            $appliedData = $query->paginate(10);

            return view('employer.emp-applied-jobseeker', compact('appliedData','totalCount'));
        }
    }
    public function shortlisted()
    {
        if (session()->has('emp_username')) {
            ...
            $query = DB::table('job_application_history')
                ...
                ->orderBy('job_application_history.update_at', 'DESC');
                $totalCount = $query->count();
            $shortlisted = $query->paginate(10);
                
              
            // return $shortlisted;
            return view('employer.emp-shortlisted-jobseeker', compact('shortlisted','totalCount'));
        }
    }
